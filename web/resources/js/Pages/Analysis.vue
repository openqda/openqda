    ...
    <AppLayout>
        <div class="lg:flex">
            ...
            <div class="w-full lg:w-2/3 p-4" id="analysis-root">
                ...
                <component :is="visualizerComponent"
                           :files="files"
            ...
            <div class="w-full lg:w-1/3 p-4">
                ...
                <div>
                    <select id="location" name="location"
                        ...
                        <option v-for="plugin in availablePlugins" :value="plugin.name">{{ plugin.title }}</option>
                    </select>
                </div>
                ...
                <table class="w-full mt-4 border-collapse border-0">
                    ...
                    <tr class="text-sm hover:bg-silver-300">
                        ...
                        <td class="py-2 tracking-wide">
                            <label for="all_files" class="cursor-pointer select-none">All files</label>
                        </td>
                    </tr>
                    <tr v-for="(file) in files" :key="file.id" class="text-sm hover:bg-silver-300">
                        <td class="py-2 text-center tracking-wide">
                            <input
                                type="checkbox"
                                class="cursor-pointer"
                        ...
                        <td class="py-2 tracking-wide" @click="checkFile($event, file.id)">
                            <label :for="file.id" class="cursor-pointer select-none">{{ file.name }}</label>
                        </td>
                    </tr>
                    </tbody>
                </table>
                ...
                <table class="w-full mt-4 border-collapse border-0">
                    ...
                    <tr v-for="(code, index) in codes" :key="code.id" class="text-sm border-0 hover:bg-silver-300">
                        ...
                        <td class="py-2 tracking-wide border-0" :style="{ backgroundColor: code.color }">
                            <label :for="code.id" class="cursor-pointer select-none">{{ code.name }}</label>
                        </td>
                    </tr>
                    </tbody>
                </table>
...
const selectVisualizerPlugin = ({ value }) => {
    ...
    visualizerComponent.value = loader === null
        ? loader
...
const updateHasSelection = debounce(() => { // TODO debounce or throttle?
    const values = []
...
const saveCSV = () => {
    ...
    const csv = createCSV({ header: ['file', 'code category', 'created by', 'created at', 'last update', 'start pos', 'end pos', 'selection'] })
