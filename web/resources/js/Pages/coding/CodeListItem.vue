  ...
  <li :class="cn('rounded-md py-2', open && 'bg-background/20' ,props.liclass)">
    <div class="flex items-center w-auto space-x-3">
      ...
      <span class="w-4 h-4 p-1 m-1" v-else></span>
        <Button
            :title="visibleSelections[code.id] ? 'Hide selections list' : 'Show selections list'"
            variant="ghost"
            size="sm"
            :class="cn('!px-1 !py-1 !my-0 w-8 text-xs hover:text-secondary', visibleSelections[code.id] && 'text-secondary')"
            :disabled="!hasTexts"
            @click="visibleSelections[code.id] = !visibleSelections[code.id]"
        >
            <BarsArrowDownIcon class="w-4 -h-4" />
            <span class="text-xs">{{ open ? (hasTexts ?? 0) : selections(code) }}</span>
        </Button>
      <div class="flex-grow tracking-wide rounded-md px-2 py-1 text-sm group" :style="`background: ${changeRGBOpacity(code.color, 1)};`">
        <button
          v-if="range?.length"
      ...
      <div v-if="hasTexts && visibleSelections[code.id]"
           :style="`border-color: ${changeRGBOpacity(code.color, 1)};`"
           class="bg-surface border text-sm ms-8 me-1 my-1 rounded"
           >
          <ul class="divide-y divide-border">
              <li v-for="selection in code.text.toSorted((a, b) => a.start - b.start)" class="p-3 hover:bg-background/20 cursor-pointer" @click="focusSelection(selection)">
                  <div class="w-full flex items-center justify-between tracking-wider text-xs font-semibold">
                      ...
                      <button class="p-2" @click="Selections.deleteSelection(selection)">
                          <TrashIcon class="w-3 h-3 hover:text-destructive" />
                      </button>
                  </div>
