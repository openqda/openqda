<template>
    <div class="block text-right">
        ...
        <input id="portrait-grid-size"
               class="rounded focus:ring-1 focus:ring-inset focus:ring-cerulean-700"
               type="number" v-model="gridSize" min="1" max="12">
    </div>
    ...
    <div v-for="(file) in files"
         class="border border-silver-300 p-2 col-span-1">
            ...
            <div class="flex flex-wrap">
                <span v-for="(entry, index) in segments.get(file.id)"
                     :key="`${file.id}-${index}`"
                      :title="`${entry.segment.start}-${entry.segment.end};\n\n${entry.segment.text.substring(0, 250)}...`"
                     class="m-1">
                    <EllipsisHorizontalCircleIcon :style="{ color: entry.color, backgroundColor: entry.color  }" class="w-4 h-4" />
                </span>
            </div>
            <div v-if="!segments.has(file.id)" class="ml-2 mt-2 p-2 bg-silver-100">No codes</div>
        </div>
    </div>
</template>
...
import { XMarkIcon, EllipsisHorizontalCircleIcon, DocumentTextIcon } from '@heroicons/vue/24/solid'
...
const getSegmentsForFile = (file) => {
    const codes = props.codes.filter(code => (
