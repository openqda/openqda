<template>
    <div class="p-3 mb-5">
        <table class="table-auto word-cloud-settings w-full">
            <thead>
            <tr>
                <th scope="col" class="text-left  text-xs font-medium uppercase text-silver-500">Height</th>
                <th scope="col" class="text-left  text-xs font-medium uppercase text-silver-500">Min word length</th>
                <th scope="col" class="text-left text-xs font-medium uppercase text-silver-500">Scale</th>
                <th scope="col" class="text-left text-xs font-medium uppercase text-silver-500">+ Size</th>
                <th scope="col" class="text-left text-xs font-medium uppercase text-silver-500">
                    <Cog6ToothIcon
                        v-if="generating"
            ...
            <tr>
                ...
                <td >
                    <input type="range" v-model="scaleFactor" min="1" max="50" step="1">
                ...
                <td>
                    <Button color="cerulean" @click="rebuild" label="Refresh" :disabled="generating"/>
                </td>
            </tr>
            </tbody>
        </table>
    ...
    <div ref="resizeRef" class="cloud-root" :style="{height: windowHeight + 'px' }">
...
const props = defineProps(['files', 'codes', 'checkedFiles', 'checkedCodes', 'hasSelections', 'api'])
...
function rebuild () { seed.value = Math.random() }

watch(minHeight, debounce((val) => {
...
onMounted(() => {
    ...
    const _setupDebounced = debounce(({ width, height, wordsList, scale }) => {
        layout.size([width, height])
            .words(wordsList.map(function ([d, count]) {
                return { text: d, size: (count * scale.factor) + scale.addition }
            }))
            .padding(5)
        ...
        for (const file of files) {
            if (checkedFiles.get(file.id)) {
                for (const code of codes) {
                    if (checkedCodes.get(code.id) && code.text.some(t => t.source_id === file.id)) {
