...
import {ChartPieIcon, DocumentTextIcon, Square3Stack3DIcon, SquaresPlusIcon} from '@heroicons/vue/20/solid';
...
watch(usersInChannel, (newValue, oldValue) => {
    usersInPages.value = newValue; // Correctly update the ref's value
}, {deep: true}); // Use deep watch to react to nested changes
...
const selectTab = (selectedTab) => {
    tabs.value.forEach((tab) => {
        tab.current = tab.name.toLowerCase().includes(selectedTab.name.toLowerCase());
    });
};
        ...
        <nav class="flex -mb-px" aria-label="Tabs">
                <span
                    ...
                    <a :href="route('projects.index')" title="Return to Projects Overview">
                        <img class="nav-logo antialiased" :src="$page.props.logo" alt="OpenQDA Logo" />
                    </a>
                </span>
            <Link
                v-for="tab in tabs"
            ...
            tab.current
                ...
                'ml-3 hidden rounded-full py-0.5 px-2.5 text-xs font-medium md:inline-block'
                    ...
                    <div class="w-full mt-1 h-6 ">
                        <div v-if="usersInPages && usersInPages.length > 0" class="flex justify-center -space-x-2 ">
