    ...
    <table class="w-full mt-4 border-collapse border-0 overflow-show">
        ...
        <tr class="border-b border-gray-200 align-middle" :class="props.rowClass">
            ...
            <th scope="col"
                ...
                <a href @click.prevent="sort('name')" class="flex tracking-wider" title="Sort by file name">
                    <span>File</span>
                    <span>
                            <ChevronUpIcon class="h-4 w-4 gray-500"
                                           v-if="sorter.key === 'name' && sorter.ascending === true"/>
                            <ChevronDownIcon class="h-4 w-4 gray-500"
                                             v-if="sorter.key === 'name' && sorter.ascending === false"/>
                        </span>
                </a>
            </th>

            <th scope="col" class="p-2 text-xs font-medium uppercase text-gray-500 sm:pl-0">
                <a href @click.prevent="sort('type')" class="flex tracking-wider justify-center text-center" title="Sort by data type">
                    <span>Type</span>
                    <span>
                            <ChevronUpIcon class="h-4 w-4 gray-500"
            ...
            <th scope="col" class="p-2  text-xs font-medium uppercase tracking-wider text-gray-500 sm:pl-0">
                <a href @click.prevent="sort('date')" class="flex justify-center" title="Sort by upload date">
                    <span>Date</span>
                    <span>
                            <ChevronUpIcon class="h-4 w-4 gray-500"
            ...
            <th scope="col"
                ...
                <a href @click.prevent="sort('user')" class="flex justify-center" title="Sort by username">
                    <span>By</span>
                    <span>
                            <ChevronUpIcon class="h-4 w-4 gray-500"
            ...
            <th scope="col" v-show="$props.actions?.length"
                class="p-2 text-center  text-xs font-medium uppercase text-gray-500 sm:pl-0">
        ...
        <tbody>
        <tr v-for="(document, index) in $props.documents" :key="document.id"
        ...
        >
            ...
            <td class="py-4 flex items-center w-auto gap-2">
                <a @click="emit('select', document)"
                   :class="document.converted ? 'hover:text-porsche-400 cursor-pointer' : 'text-silver-300 pointer-events-none' "
                   class="tracking-wider">
                    {{ document.name }}
                </a>
                <div v-if="!document.converted && !document.isConverting"
                     class="flex items-center bg-yellow-100 text-yellow-800 font-semibold px-2 py-1 mx-2 text-xs">
                    <ExclamationTriangleIcon class="w-3 h-3 text-yellow-800 mr-1"></ExclamationTriangleIcon>
                    <span class="hidden md:block whitespace-nowrap">not ready to be coded - retry elaboration</span>
                </div>


                <div v-if="document.isConverting"
            ...
            <td class="py-2">
                ...
                <div :title="dataTypeTitle(document.type)" class="w-full text-center">
                    <DocumentTextIcon v-if="document.type === 'text'" class="h-4 w-4 gray-500 ml-auto mr-auto" />
                </div>
            </td>
            ...
            <td class="py-2 text-center tracking-wider w-8 h-8">
                ...
                <img v-if="document.userPicture"
                     class="object-cover w-full h-8 rounded-full"
                     :src="document.userPicture"
                     :title="document.user"
                     :alt="document.user"/>
                <div v-else class="flex items-center justify-center w-full h-full rounded-full bg-gray-200">
                    <span class="text-gray-500">{{ document.user }}</span>
                </div>

            </td>
            <td v-show="$props.actions?.length" class="py-2 text-center tracking-wider justify-center align-middle items-center relative">
                <button @click="toggleMenu(document.id)" class="hover:text-porsche-400 focus:text-porsche-400 cursor-pointer menu-toggle">
                    <EllipsisVerticalIcon class="w-4 h-4 menu-toggle z-0"></EllipsisVerticalIcon>
                </button>

                <div v-show="isMenuOpen(document.id)"
                ...
                >
                    <span v-for="(action, actionIndex) in $props.actions" :key="action.id" :title="action.title"
                          class="items-center">
                        <button
                            v-if="!(action.id === 'retry-conversion' && (document.isConverting || document.converted))"
