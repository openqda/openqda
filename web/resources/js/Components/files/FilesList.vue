...
import { useLocalSearch } from '../../utils/search/useLocalSearch.js';
...
const { searchTerm, searchFilter } = useLocalSearch({
  fields: {
    name: String,
    date: String,
    id: String,
...
const docs = computed(() =>
  toArray(props.documents).filter(searchFilter.value)
);
...
<template>
  <div class="block w-full">
    ...
    <div v-if="!docs?.length" class="text-sm text-foreground/50">
      <p v-if="searchTerm?.length">No entries matched for your search.</p>
    </div>
    <table
      :class="cn('w-full border-collapse mt-5', props.fixed && 'table-fixed')"
    >
      <thead>
          ...
          <td
            ...
            :class="
              cn(
                ...
                document.selected && 'font-semibold'
              )
            "
