    ...
    <WizardStep name="localUpload" title="Add your files">
        <Dropzone
            ...
            <FileUploadButton
                ...
                accept=".txt,.md,.rtf,.html,.pdf"
                @file-added="addFiles">
                <span class="cursor-pointer hover:underline">Drop the files you want to upload here or click to select files.</span>
            </FileUploadButton>
            </div>
        </Dropzone>
        <table v-if="localFiles.length" class="table my-5 text-sm">
            <tr v-for="(file, index) in localFiles" :key="file.name">
                ...
                <td><Button variant="ghost" size="sm" @click.prevent="localFiles.splice(index, 1)">&times;</Button></td>
            </tr>
        </table>
        <Button v-if="localFiles.length" size="lg" class="w-full mt-5" @click.prevent="uploadFiles">Start upload</Button>
    </WizardStep>
  </Wizard>
</template>
