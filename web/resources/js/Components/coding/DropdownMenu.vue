<template>
    <div class="absolute z-50 right-0 w-44 mt-2 bg-white rounded shadow-lg ring-1 ring-black ring-opacity-5"
         role="menu"
         aria-orientation="vertical"
         aria-labelledby="options-menu">
        <button class="w-full flex items-center justify-start py-2 px-4 hover:bg-gray-200 rounded" role="menuitem">
            <SwatchIcon class="w-4 h-4 mr-2 text-black"/>
            <InputLabel>Change Color</InputLabel>
            <input type="color"

                   :value="rgbToHex(extractRGB(code.color).r, extractRGB(code.color).g, extractRGB(code.color).b)"
