export const updateLineNumbers = async (editorId, lineNumbersId, charCount, debug = false) => {
    ...
    for (let i = 0; i < textArr.length; i++) {
        ...
        if (debug && currentHeight > lineHeightTemp) tempSpan.style.backgroundColor = "red";
    ...
    actualCharsPerLine.forEach((charsInLine, index) => {
        ...
        if (lineNumbersForThisLine.length > 0) {
            const firstNumber = String(lineNumbersForThisLine[0]).padStart(padLength, ' ');
            const lastNumber = String(lineNumbersForThisLine[lineNumbersForThisLine.length - 1]).padStart(padLength, ' ');
    ...
    if (debug) {
        ...
        console.log("Sum of Characters in array:", actualCharsPerLine.reduce((partialSum, a) => partialSum + a, 0));
